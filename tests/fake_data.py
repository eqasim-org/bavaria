...
##########################################################################
output_path = (
    "C:/Users/arthur.burianne/Documents/sources_data/fake_data_munich"
)
...
# il faut re créer la bonne construction de numéro de département dans fake data
# il faut déclarer le département en + dela région dans fake data


# load geographical data

local_name = output_path + "/pickles_mun/"
region = "09"
...
with zipfile.ZipFile(
    "%s/filosofi_2019/indic-struct-distrib-revenu-2019-COMMUNES.zip"
    % output_path,
    "w",
) as archive:
    with archive.open("FILO2019_DISP_COM.xlsx", "w") as f:
        df_income.to_excel(f, sheet_name="ENSEMBLE", startrow=5, index=False)
...
for household_index in range(CENSUS_HOUSEHOLDS):
    ...
    department = df.at[token, "department"]
    ...
    destination_department = df[
        df["municipality"] == destination_municipality
    ]["department"].values[0]
...
ids = [
    "BATIMENT{:016d}".format(n)
    for n in random.randint(1000, 1000000, observations)
]
...
df_bdtopo.set_geometry(
    df_bdtopo.buffer(40), inplace=True, drop=True, crs="EPSG:25832"
)
...
with py7zr.SevenZipFile(
    ...
    archive.write(
        "{}/bdtopo_idf/content.gpkg".format(output_path),
        "content/content.gpkg",
    )
...
df_sirene.to_csv(
    ...
    compression={
        "method": "zip",
        "archive_name": "StockEtablissement_utf8.csv",
    },
)
...
codes_com = df_codes["commune_id"].iloc[
    random.randint(0, len(df_iris), observations)
]
